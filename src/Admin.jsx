...
const S = {
  ...
  badge:   (color) => ({ display:'inline-flex', alignItems:'center', gap:4, padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:700, background: color==='green'?'#052e16':color==='yellow'?'#1c1917':'#1e1b4b', color: color==='green'?'#4ade80':color==='yellow'?'#fb923c':'#818cf8' }),
};
    ...
    <div>
      ...
      <div style={S.card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          ...
          <button onClick={()=>setView('projects')} style={{ ...S.btn, background:'transparent', color:'#60a5fa', padding:'4px 8px' }}>
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:16, alignItems:'center' }}>
        <div style={{ position:'relative', flex:1, maxWidth:280 }}>
          ...
          <input style={{ ...S.input, paddingLeft:32 }} placeholder="搜尋案例名稱..." value={search} onChange={e=>setSearch(e.target.value)}/>
        </div>
        <div style={{ display:'flex', gap:4, background:'#0f172a', padding:4, borderRadius:8 }}>
          {cats.map(c=>(
            <button key={c} onClick={()=>setFilter(c)} style={{ ...S.btn, padding:'5px 12px', background: filter===c?'#2563eb':'transparent', color: filter===c?'#fff':'#94a3b8' }}>{c}</button>
          ))}
        </div>
      </div>
          ...
          <tr key={p.id} style={{ borderBottom:'1px solid #1e293b' }}>
            ...
            <td style={{ padding:'10px 12px' }}>
              <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                <span style={S.badge(p.published?'green':'yellow')}>{p.published?'✓ 已發布':'草稿'}</span>
            ...
            {!compact && (
              <td style={{ padding:'10px 12px' }}>
                <div style={{ display:'flex', gap:6 }}>
                  <button style={{ ...S.btn, background:'#1e3a5f', color:'#60a5fa', padding:'5px 10px' }} onClick={()=>setView&&setView('edit')}>
                    <Pencil size={12}/>
                  </button>
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
        <button onClick={()=>setView('projects')} style={{ ...S.btn, background:'#1e293b', color:'#94a3b8', padding:'6px 12px' }}>← 返回</button>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:20 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={S.card}>
            ...
            <Row cols={1}>
              <Field label="案例名稱">
                <input style={S.input} placeholder="例：九龍灣私人住宅翻新" defaultValue={isEdit?'九龍灣私人住宅':''}/>
              </Field>
            </Row>
            <Row>
              <Field label="分類">
            ...
            <Row>
              <Field label="地點"><input style={S.input} placeholder="例：九龍灣" defaultValue={isEdit?'九龍灣':''}/></Field>
              <Field label="完成年份"><input style={S.input} type="number" defaultValue={2024}/></Field>
            </Row>
            <Row>
              <Field label="面積 (ft²)"><input style={S.input} type="number" placeholder="850" defaultValue={isEdit?850:''}/></Field>
              <Field label="預算範圍">
                <select style={S.input}><option>$50-80萬</option><option>$80-120萬</option><option>$120萬以上</option></select>
              </Field>
            </Row>
            <Field label="案例描述">
          ...
          <div style={S.card}>
            ...
            <div style={{ border:'2px dashed #334155', borderRadius:10, padding:32, textAlign:'center', cursor:'pointer', marginBottom:16, transition:'border-color 0.2s' }}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
                {[1,2,3,4].map(i=>(
                  <div key={i} style={{ position:'relative', borderRadius:8, overflow:'hidden' }}>
                    <img src={`https://placehold.co/160x100/1e293b/475569?text=圖片${i}`} style={{ width:'100%', display:'block' }}/>
                    <button style={{ position:'absolute', top:4, right:4, background:'#ef4444', border:'none', borderRadius:4, padding:2, cursor:'pointer', display:'flex', alignItems:'center' }}>
                      <X size={10} color="#fff"/>
                    </button>
                    {i===1 && <div style={{ position:'absolute', bottom:0, left:0, right:0, background:'#2563eb', fontSize:9, textAlign:'center', padding:'2px 0', fontWeight:700 }}>封面</div>}
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={S.card}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              ...
              <div onClick={()=>setPublished(!published)} style={{ width:40, height:22, borderRadius:11, background: published?'#2563eb':'#334155', cursor:'pointer', position:'relative', transition:'background 0.2s' }}>
                <div style={{ width:16, height:16, borderRadius:'50%', background:'#fff', position:'absolute', top:3, left: published?21:3, transition:'left 0.2s' }}/>
              </div>
            </div>
            {/* Featured Toggle */}
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              <span style={{ fontSize:13 }}>置頂精選</span>
              <div onClick={()=>setFeatured(!featured)} style={{ width:40, height:22, borderRadius:11, background: featured?'#f59e0b':'#334155', cursor:'pointer', position:'relative', transition:'background 0.2s' }}>
                <div style={{ width:16, height:16, borderRadius:'50%', background:'#fff', position:'absolute', top:3, left: featured?21:3, transition:'left 0.2s' }}/>
              </div>
            </div>
            <div style={{ borderTop:'1px solid #334155', paddingTop:14, display:'flex', flexDirection:'column', gap:8 }}>
              ...
              <button style={{ ...S.btn, background:'#1e293b', color:'#94a3b8', justifyContent:'center', border:'1px solid #334155' }}>
                儲存草稿
              </button>
            </div>
          </div>
          ...
          <div style={S.card}>
            ...
            <input style={{ ...S.input, marginTop:10 }} placeholder="輸入標籤後按 Enter"/>
          </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ background:'#0f172a', borderRadius:8, overflow:'hidden' }}>
              <img src="https://placehold.co/280x160/0f172a/334155?text=封面圖片" style={{ width:'100%', display:'block' }}/>
              <div style={{ padding:12 }}>
                <div style={{ fontWeight:700, fontSize:13 }}>九龍灣私人住宅</div>
                <div style={{ fontSize:11, color:'#64748b', marginTop:4 }}>現代簡約 · 850 ft² · 2024</div>
              </div>
            </div>
          </div>
        </div>
          ...
          {MOCK_INQUIRIES.map(inq=>(
            <div key={inq.id} onClick={()=>setSelected(inq)}
              style={{ display:'flex', gap:12, padding:'12px 0', borderBottom:'1px solid #1e293b', cursor:'pointer', alignItems:'flex-start',
                background: selected?.id===inq.id?'rgba(37,99,235,0.1)':undefined, borderRadius:6, paddingLeft:8 }}>
              <div style={{ width:36, height:36, borderRadius:'50%', background:'#334155', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:13, flexShrink:0 }}>
                {inq.name[0]}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:12, color:'#94a3b8', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{inq.message}</div>
              </div>
              {!inq.read && <div style={{ width:8, height:8, borderRadius:'50%', background:'#3b82f6', flexShrink:0, marginTop:4 }}/>}
            </div>
          ))}
        </div>
          ...
          {selected ? (
            <>
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <div style={{ fontSize:13, marginTop:2, background:'#0f172a', padding:10, borderRadius:8, lineHeight:1.6 }}>{selected.message}</div>
              </div>
...
export default function Admin() {
  ...
  const activeNav = view==='projects'||view==='new'||view==='edit' ? 'projects' : view==='inquiries' ? 'inquiries' : 'dashboard';
        ...
        <nav style={{ flex:1, padding:'12px 8px' }}>
          {NAV.map(({id,label,icon:Icon})=>(
            <button key={id} onClick={()=>setView(id)}
              ...
              <Icon size={15}/> {label}
              {id==='inquiries' && <span style={{ marginLeft:'auto', background:'#3b82f6', color:'#fff', borderRadius:10, padding:'1px 6px', fontSize:10 }}>1</span>}
            </button>
          ))}
        </nav>
